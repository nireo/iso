...
void
init_iso(char** volumes, size_t volume_count, char* index_path)
{
...
static void
_write_to_store(const char* key, const Entry* entry)
{
...
static void
_set_entry(const char* key, size_t keylen, const char* volume)
{
    char* err = NULL;
...
static char*
_get_entry_(const char* key, size_t keylen)
{
    char* err = NULL;
    size_t read_len = 0;
...
static int
_delete_entry(const char* key, size_t klen)
{
    char* err = NULL;
...
static char*
_pick_volume(const char* key, size_t keylen)
{
    int best_volume = 0;
...
static char*
_key_to_path(const char* key, size_t keylen)
{
...
static void
data_sender(struct mg_connection* c, int ev, void* ev_data,
    void* fn_data)
{
    if (ev == MG_EV_OPEN) {
